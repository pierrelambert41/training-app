...
| Queue de retry IA | _(consommé TA-141)_ | `processPendingAICalls` (worker batch 5, max 3 tentatives) + `AIQueueBridge` au retour réseau | Types session_summary et block_summary traités ; plateau/explain_adjustment → failed direct (à la demande). generateProgram/regenerateBlock exclus (TA-146). |
| UI "Pourquoi ?" | _(consommé TA-139)_ | `RecommendationWhy` branché dans `session-recommendations.tsx` (écran fin de séance) | Reste : écran Aujourd'hui si besoin |
...
| UI résumé de bloc (affichage) | _(consommé TA-140)_ | `useStoredBlockSummary` + `AISummaryCard` branchés dans `active-block-screen.tsx` | Reste : bouton de *génération* à la demande (`useBlockSummary` TA-138) à brancher quand le flow fin de bloc existera. |
