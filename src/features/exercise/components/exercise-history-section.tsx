...
type Props = {
  ...
  /** Unité d'affichage de l'exercice — les loads stockés sont en kg canonique. */
  unit?: WeightUnit;
};
...
function formatBestSet(bestSet: ExerciseSessionHistory['bestSet'], unit: WeightUnit): string {
  const parts: string[] = [];
  if (bestSet.load !== null) parts.push(formatWeight(bestSet.load, unit));
  if (bestSet.reps !== null) parts.push(`${bestSet.reps} reps`);
  if (bestSet.rir !== null) parts.push(`@ RIR ${bestSet.rir}`);
  return parts.length > 0 ? parts.join(' × ') : '—';
}

function HistoryRow({ entry, unit }: { entry: ExerciseSessionHistory; unit: WeightUnit }) {
  return (
    <View
    ...
    >
      ...
      <AppText variant="caption" className="flex-1 text-right text-content-primary">
        {formatBestSet(entry.bestSet, unit)}
      </AppText>
    </View>
  );
}

export function ExerciseHistorySection({ history, unit = 'kg' }: Props) {
  if (history.length === 0) {
    return (
  ...
  return (
    <View testID="exercise-history-list">
      {history.map((entry) => (
        <HistoryRow key={entry.sessionId} entry={entry} unit={unit} />
      ))}
    </View>
  );
}
