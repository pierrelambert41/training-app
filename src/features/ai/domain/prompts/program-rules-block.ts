...
${input.maxSessionDurationMin !== null ? `8. Durée de séance cible : ${input.maxSessionDurationMin} min, tolérance acceptée jusqu'à ${input.maxSessionDurationMin + SESSION_DURATION_TOLERANCE_MIN} min (vérifié par le validateur : ~${SESSION_WARMUP_MIN} min d'échauffement + ~${SESSION_MIN_PER_SET} min par série, repos inclus). Budget : maximum ${Math.floor((input.maxSessionDurationMin + SESSION_DURATION_TOLERANCE_MIN - SESSION_WARMUP_MIN) / SESSION_MIN_PER_SET)} séries par séance — compte tes séries avant de répondre. Si le budget ne permet pas le plancher pour tous les muscles : restreins le nombre de groupes ciblés directement (priorités d'abord, le reste couvert indirectement par les polyarticulaires) plutôt que de dépasser la fenêtre.` : '8. Pas de contrainte de durée de séance.'}
